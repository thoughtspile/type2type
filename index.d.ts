...
export namespace TQueue {
  ...
  export type peek<Q extends AnyTStack> = Q extends [infer Head, ...infer _]
    ? Head
    : never;
  export type pop<Q extends AnyTStack> = Q extends [infer _, ...infer Tail]
    ? TQueue<Tail>
    : Q;
...
export namespace TStack {
  ...
  export type peek<Q extends AnyTQueue> = Q extends [infer Head, ...infer _]
    ? Head
    : never;
  export type pop<Q extends AnyTQueue> = Q extends [infer _, ...infer Tail]
    ? TQueue<Tail>
    : Q;
  export type size<Q extends AnyTQueue> = Q["length"];
  export type empty<Q extends AnyTQueue> = Equals<size<Q>, 0>;
...
export namespace TSet {
  export type has<Set extends AnyTSet, El> = {
    [T in keyof Set]: Equals<Set[T], El>;
  }[number];
  export type add<Set extends AnyTSet, El> = TSet.has<Set, El> extends never
    ? TSet<[...Set, El]>
    : Set;
  export type remove<Set extends AnyTSet, El> = Set extends [
    infer Head,
    ...infer Tail,
  ]
    ? Equals<Head, El> extends never
      ? [Head, ...remove<Tail, El>]
      : remove<Tail, El>
    : Set;
  ...
  > = S1 extends [infer Head, ...infer Tail]
    ? has<S2, Head> extends never
      ? intersection<Tail, S2>
      : TSet<[Head, ...intersection<Tail, S2>]>
    : TSet;
  ...
  export type select<S extends AnyTSet, Pred = unknown> = S extends [
    infer Head,
    ...infer Tail,
  ]
    ? Head extends Pred
      ? [Head, ...select<Tail, Pred>]
      : select<Tail, Pred>
    : S;
...
export namespace TMap {
  export type get<Q extends AnyTMap, Key> = {
    [T in keyof Q]: Equals<Q[T][0], Key> extends never ? never : Q[T][1];
  }[number];
  export type has<Q extends AnyTMap, Key> = {
    [T in keyof Q]: Equals<Q[T][0], Key>;
  }[number];
  export type set<Q extends AnyTMap, Key, Value> = TMap.has<
    Q,
    Key
  > extends never
    ? [...Q, [Key, Value]]
    : {
        [T in keyof Q]: Equals<Q[T][0], Key> extends never
          ? Q[T]
          : [Key, Value];
      };
  export type remove<Q extends AnyTMap, Key> = Q extends [
    infer Head extends TMapItem,
    ...infer Tail extends TMapItem[],
  ]
    ? Equals<Head[0], Key> extends never
